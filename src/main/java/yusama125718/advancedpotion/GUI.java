...
public class GUI {
    ...
    public static void addrecipeGUI(Player player){
        Inventory inv = Bukkit.createInventory(null,45, Component.text("[AdvPot]Add Recipe"));
        for (int i = 1;i <= 54;i++) if (i != 12 && i != 24 && i != 25 && i != 26 && i != 30) inv.setItem(i,getItem(Material.WHITE_STAINED_GLASS_PANE,1,"",0));
        inv.setItem(21,getItem(Material.BLACK_STAINED_GLASS_PANE,1,"上に材料(上)を置く",0));
        inv.setItem(33,getItem(Material.BLACK_STAINED_GLASS_PANE,1,"上に完成品(1)を置く",0));
        inv.setItem(34,getItem(Material.BLACK_STAINED_GLASS_PANE,1,"上に完成品(2)を置く",0));
        inv.setItem(35,getItem(Material.BLACK_STAINED_GLASS_PANE,1,"上に完成品(3)を置く",0));
        inv.setItem(39,getItem(Material.BLACK_STAINED_GLASS_PANE,1,"上に材料(下)を置く",0));
        inv.setItem(41,getItem(Material.RED_STAINED_GLASS_PANE,1,"追加する",0));
        player.openInventory(inv);
    }
}
